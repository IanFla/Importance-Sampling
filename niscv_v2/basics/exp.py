...
class Exp:
    ...
    def __estimate(self, weights, funs, name=None, mode=None, reg=None):
        if mode != 'regress':
            ...
            self.mu = np.sum(y - X.dot(reg[3])) / np.sum(w - X.dot(reg[4])) \
                if self.sn else np.mean(y - X.dot(reg[3]))
    ...
    def resampling(self, size_kn, ratio, resample=True, bootstrap=True):
        ...
        if resample:
            ...
            if bootstrap:
                ...
                self.others.append(self.weights_kn.size)
            else:
    ...
    def regression_estimation(self, mode):
        ...
        if mode == 1:
            ...
            if self.sn:
                ...
                self.others.extend([self.reg_y.score(X, y), self.reg_w.score(X, w)])
                self.__estimate(None, None, 'RIS', mode='regress', reg=[X, w, y, self.reg_y.coef_, self.reg_w.coef_])
            else:
                self.disp('Regression R2: {:.4f}'.format(self.reg_y.score(X, y)))
                self.others.append(self.reg_y.score(X, y))
                self.__estimate(None, None, 'RIS', mode='regress', reg=[X, w, y, self.reg_y.coef_])
            ...
            if self.sn:
                ...
                self.__estimate(None, None, 'RIS', mode='regress', reg=[X2, w2, y2, reg1_y.coef_, reg1_w.coef_])
                mu1 = self.mu.copy()
                self.__estimate(None, None, 'RIS', mode='regress', reg=[X1, w1, y1, reg2_y.coef_, reg2_w.coef_])
                mu2 = self.mu.copy()
            else:
                self.disp('Regression 1 R2: {:.4f}'.format(reg1_y.score(X2, y2)))
                self.disp('Regression 2 R2: {:.4f}'.format(reg2_y.score(X1, y1)))
                self.__estimate(None, None, 'RIS', mode='regress', reg=[X2, w2, y2, reg1_y.coef_])
                mu1 = self.mu.copy()
                self.__estimate(None, None, 'RIS', mode='regress', reg=[X1, w1, y1, reg2_y.coef_])
                mu2 = self.mu.copy()

            self.result.append((mu1 + mu2) / 2)
            self.disp('RIS* est: {:.4f}'.format((mu1 + mu2) / 2))

    def likelihood_estimation(self, lim=20, sp=5):
        ...
        zetas = utils.newton(gradient, hessian, zeta0, lim=lim, sp=sp)
        ...
        for i, zetai in enumerate(zetas):
            ...
            self.__estimate(weights, self.funs_, 'MLE({})'.format(sp * i))
...
def experiment(dim, size_est, sn, show, size_kn, ratio, bootstrap):
    ...
    exp.likelihood_estimation(lim=20, sp=5)
    return exp.result
...
def main(sn, bt):
    ...
    for i in range(100):
        print(i + 1)
        np.random.seed(1997 * 1107 * i)
        result = experiment(dim=5, size_est=20000, sn=sn, show=False, size_kn=500, ratio=20, bootstrap=bt)
...
if __name__ == '__main__':
    R = [main(False, True),
         ...
         main(False, False),
         main(True, False)]
    with open('../data/test/test1', 'wb') as file:
        pickle.dump(R, file)
